-- ==========================================
-- PRAIDSC HUB ULTIMATE V1.0 (PROVEN & WORKING)
-- TELEGRAM: @PraidSC | KEY: vadereyownerpraidsc
-- ==========================================
...
-- Безопасный родитель для GUI (защита от багов инжектора)
local TargetGui = CoreGui
local successGui = pcall(function() local test = Instance.new("Folder", CoreGui) test:Destroy() end)
if not successGui then
    TargetGui = LocalPlayer:WaitForChild("PlayerGui")
end

-- Защита от повторного запуска
if TargetGui:FindFirstChild("PraidSC_Hub_V3") or TargetGui:FindFirstChild("PraidSC_KeySystem") then
    return
end

-- ------------------------------------------
-- 1. KEY SYSTEM (ОКНО КЛЮЧА)
-- ------------------------------------------
...
local function LoadMainScript()
    ...
    Title.Text = "PraidSC HUB V1.0 | TG: " .. TELEGRAM_LINK
    ...
    for i, catName in ipairs(categories) do
        ...
        CatFrame.CanvasSize = UDim2.new(0, 0, 0, 430)
    ...
    AddToggle("🏎️ Гонки", "6. Авто-Сброс Персонажа", "AutoReset", function()
        if LocalPlayer.Character then LocalPlayer.Character:BreakJoints() end
    end)
    AddToggle("🏎️ Гонки", "7. Разворот Камеры 180°", "CamFlip", function(st)
        if st then Workspace.CurrentCamera.CFrame = Workspace.CurrentCamera.CFrame * CFrame.Angles(0, math.rad(180), 0) end
    end)
    AddToggle("🏎️ Гонки", "8. Импульсный Кликер", "PulseClick", function() end)
    AddToggle("🏎️ Гонки", "9. Зафиксировать Позу (Anchor)", "FreezeChar", function(st)
        if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
            LocalPlayer.Character.HumanoidRootPart.Anchored = st
        end
    end)
    AddToggle("🏎️ Гонки", "10. Перезаход на Сервер", "RejoinServer", function(st)
        if st then TeleportService:Teleport(game.PlaceId, LocalPlayer) end
    end)

    AddToggle("🏃 Движение", "1. Спидхак (WalkSpeed 120)", "WalkSpeed", function(st)
    ...
    AddToggle("🏃 Движение", "7. Отключить Садиться (NoSit)", "NoSit", function(st)
        if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid") then
            LocalPlayer.Character.Humanoid:SetStateEnabled(Enum.HumanoidStateType.Seated, not st)
        end
    end)
    AddToggle("🏃 Движение", "8. Включить ShiftLock", "ShiftLock", function(st)
        LocalPlayer.DevEnableMouseLock = st
    end)
    AddToggle("🏃 Движение", "9. Режим Парения (Platform)", "AirWalk", function(st)
    ...
    AddToggle("🏃 Движение", "10. Сохранить / Загрузить Точку", "SavePos", function(st)
        ...
        elseif not st and _G.SavedCFrame and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
            LocalPlayer.Character.HumanoidRootPart.CFrame = _G.SavedCFrame
        end
    end)

    AddToggle("🎨 Графика", "1. FPS Booster", "FPSBoost", function(st)
    ...
    AddToggle("🛠️ Утилиты", "1. FOV 90", "FOV90", function(st)
        Workspace.CurrentCamera.FieldOfView = st and 90 or 70
    end)
    AddToggle("🛠️ Утилиты", "2. FOV 120", "FOV120", function(st)
    ...
    AddToggle("🛠️ Утилиты", "7. Скопировать Координаты", "CopyCF", function(st)
        if st and setclipboard and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
            setclipboard(tostring(LocalPlayer.Character.HumanoidRootPart.CFrame))
        end
    end)
    AddToggle("🛠️ Утилиты", "8. Скрыть GUI Игры", "HideGameUI", function(st)
        local pGui = LocalPlayer:FindFirstChildOfClass("PlayerGui")
        if pGui then
            for _, gui in pairs(pGui:GetChildren()) do
                if gui:IsA("ScreenGui") and gui.Name ~= "PraidSC_Hub_V3" and gui.Name ~= "PraidSC_KeySystem" then
                    gui.Enabled = not st
                end
            end
        end
    end)
    AddToggle("🛠️ Утилиты", "9. Взять все Предметы", "AutoEquipTool", function(st)
        if st then
    ...
    AddToggle("🛠️ Утилиты", "10. Удалить Ограничения Камеры", "UnlocksCam", function(st)
        if st then LocalPlayer.CameraMinZoomDistance = 0.5 LocalPlayer.CameraMaxZoomDistance = 99999 end
    end)
